/* 03sep22 Software Lab. Alexander Burger */
...
function renderCanvas(cvs, lst) {
   ...
   for (i = 0; i < lst.length; ++i) {
      ...
      case 18:  // (csClip)
         ctx.clip();
         break;
      case 19:  // (csDef Key DX DY Lst), (csDef Key)
         if (!cvs.pre)
            cvs.pre = new Array();
      ...
      case 21:  // (csDrawDots DX DY Lst)
         ...
         break;
      case 22:  // (csDrawImage Img X Y Lst DX DY)
         var img = new Image();
         img.src = cmd[1];
         (function (img, cmd) {
            img.onload = function() {
...
function drawCanvas(id, dly, flg) {
   ...
   req.onload = function() {
      ...
      if (ele.dly == 0)
         requestAnimationFrame(function() {drawCanvas(id, dly, flg)});
      else if (ele.dly > 0)
         setTimeout(function() {drawCanvas(id, dly, flg)}, ele.dly);
      ele.dly = dly;
   }
   try {req.send(null);}
   catch (e) {
